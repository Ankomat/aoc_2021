...
fn day6_part1() {
	...
	let v: Vec<_> = input.trim().split(',').map(|x| x.parse::<usize>().unwrap()).collect();
    let mut iter = v.iter();
    let mut population = vec![0;9];

    // make vector-map of fish population
    loop {
        match iter.next() {
            Some(fish) => {
                population[*fish] += 1;
    ...
    for i in 1..257 {
        let temp = population.remove(0);
        // println!("remove first element: {:?}", population);
        population[6] += temp;
        // println!("add to 6 days: {:?}", population);
        population.push(temp);
        println!("after {} days: {:?}", i, population);
    }
    let sum: i64 = population.iter().sum();
